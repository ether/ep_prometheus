# Contributing to ep_prometheus

Thanks for helping improve `ep_prometheus` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_prometheus` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_prometheus from your local clone
pnpm run plugins i --path ../ep_prometheus

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_prometheus/` are picked up after a server restart.
...
* Etherpad version, ep_prometheus version, browser + OS
* Steps to reproduce
